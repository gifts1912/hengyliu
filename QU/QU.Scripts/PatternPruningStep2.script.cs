...
public class AggregatedQueryClicksReducer : Reducer
{
    ...
    public override IEnumerable<Row> Reduce(RowSet input, Row output, string[] args)
    {
        ...
        foreach (Row row in input.Rows)
        {
            ...
            urlClicks.ImpressionCount = row["impressionCnt"].Long;
            dictUrlClicks[url] = urlClicks;
            totalClickCnt += clickCnt;
            totalSatCnt += satCnt;
            totalImpressionCnt += urlClicks.ImpressionCount;
        }
        string aggregatedUrlClicks = "";
        count = 0;
        foreach (var kvp in dictUrlClicks)
        {
            string urlClicksStr = string.Format("{0},{1},{2},{3}", kvp.Key, kvp.Value.ClickCount, kvp.Value.SatCount, kvp.Value.ImpressionCount);
            if (count++ == 0)
            {
...
public class CalculateClickOverlapProcessor : Processor
{
    public override Schema Produces(string[] columns, string[] args, Schema input)
    {
        Schema outSchema = new Schema("Query, ParaphrasedQuery, PatternId, TotalClickCnt:long, TotalSatCnt:long, TargetTotalClickCnt:long, TargetTotalSatCnt:long, BinaryOverlap:int, FloatOverlap:float, IsValidSatOverlap:bool, BinarySatOverlap:int, FloatSatOverlap:float");
        return outSchema;
    }

    private bool TryParseUrlClickInfo(string info, out string url, out UrlClickInfo urlClick)
    {
        url = "";
        long clickCount = 0, satCount = 0, impressionCnt = 0;
        urlClick = new UrlClickInfo();
        ...
        if (!long.TryParse(items[2], out satCount))
            satCount = 0;
        if (!long.TryParse(items[3], out impressionCnt))
            impressionCnt = 0;

        urlClick.SatCount = satCount;
        urlClick.ClickCount = clickCount;
        urlClick.ImpressionCount = impressionCnt;
    ...
    public override IEnumerable<Row> Process(RowSet input, Row output, string[] args)
    {
        foreach (Row row in input.Rows)
        {
            ...
            output["TotalClickCnt"].UnsafeSet(totalClickCount);
            output["TotalSatCnt"].UnsafeSet(totalSatCount);
            output["TargetTotalClickCnt"].UnsafeSet(targetTotalClickCount);
            output["TargetTotalSatCnt"].UnsafeSet(targetTotalSatCount);

            string strUrlClicks = row["AggregatedUrlClicks"].String;
            foreach (var urlClick in strUrlClicks.Split('|'))
            {
                string url;
                UrlClickInfo urlClickInfo;
                if (!TryParseUrlClickInfo(urlClick, out url, out urlClickInfo))
                    continue;
                dictUrlClicks[url] = urlClickInfo;
            }
            ...
            foreach (var urlClick in strTargetUrlClicks.Split('|'))
            {
                ...
                if (dictUrlClicks.ContainsKey(url))
                {
                    if (urlClickInfo.ClickCount > 0)
                    {
                        overlapClickCount += dictUrlClicks[url].ClickCount;
                    }
                    if (urlClickInfo.SatCount > 0)
                    {
                        overlapSatCount += dictUrlClicks[url].SatCount;
                    }
                }
            }
