...
{
    /// <summary>
    /// Build a reformulation pattern trie for fast matching and replacing.
    /// </summary>
    public class ReformulationPatternTrie
    {
        public class Node
        {
            ...
            /// </summary>
            private List<ReformulationPattern> reformulations;
            public List<ReformulationPattern> Reformulations
            {
                get { return reformulations; }
                set { reformulations = value; }
            }
        ...
        /// <returns></returns>
        public Node BuildTree(IEnumerable<string> patterns, Dictionary<string, List<ReformulationPattern>> dict)
        {
            Node root = new Node()
            {
                Content = Start,
            ...
            foreach (string p in patterns)
            {
                ...
                {
                    Node end = new Node 
                    { Content = End, Parent = currRoot, Children = null, Pattern = p };
                    List<ReformulationPattern> reformulations;
                    if (!dict.TryGetValue(p, out reformulations))
                        reformulations = null;
                    end.Reformulations = reformulations;
                ...
                else
                {
                    ...
                    if (!dict.TryGetValue(p, out reformulations))
                        reformulations = null;
                    currRoot.Reformulations = reformulations;
                }
            }
        ...
        public class MatchInfo
        {
            ...
            public List<ReformulationPattern> reformPatterns;
        }
            ...
            else if (start == items.Length - 1)
            {
                if (currRoot.HasChild(End))
                {
                    ...
                    matches.Add(new MatchInfo 
                                { reformPatterns = currRoot.GetChild(End).Reformulations, 
