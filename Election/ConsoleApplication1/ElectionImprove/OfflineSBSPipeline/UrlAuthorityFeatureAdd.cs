...
namespace ElectionImprove.OfflineSBSPipeline
{
    ...
    public class UrlAuthorityFeatureAdd
    {
        ...
        private static int urlIdx = 1;
        private static int urlRegIdx = 2;
        private static int urlScoreIdx = 3;
        ...
        private static void Preprocessing(string pat2topsite, string keyDef = "0", string keySortDef = "-1")
        {
            ...
            using (StreamReader streamReader = new StreamReader(pat2topsite))
            {
                while (!streamReader.EndOfStream)
                {
                    ...
                    string url = strArray[urlIdx];
                    url = url.Trim(new char[] { '/', ' ' });
                    m_topSiteTable[keySort].Add(new TopSiteItem(url, strArray[urlRegIdx], strArray[urlScoreIdx]));
        ...
        public static void Run(string[] args)
        {
            ...
            else
            {
                if (args.Length == 0)
                {
                    ...
                    args[4] = "m:PatternEngineId"; //PatColumn
                    args[5] = "0"; // the key is the value combination of 0th and 1th column.
                }
                string keyColCombination = args[5];
                string PEUrlFile = args[1];
                Preprocessing(PEUrlFile, keyColCombination, "-1"); // process pattern engine template URL
                m_isMatchingHost = args[3] == "true";
                string keyFeature = args[4]; //m:QueryPattern
                string keySortFeature = "-1";
                int queryIdx = -1;
